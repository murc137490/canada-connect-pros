...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let toEmail = "";
    let language: Language = normalizeLanguage(body.language);
    ...
    if (type.startsWith("booking_") || type === "admin_new_booking") {
      ...
      if (type === "admin_new_booking") {
        toEmail = ADMIN_EMAIL;
        language = normalizeLanguage(body.language ?? Deno.env.get("ADMIN_EMAIL_LANGUAGE") ?? "en");
      } else {
        toEmail = bookingContext.clientEmail;
        language = bookingContext.clientLanguage;
      }
