-- `cycle_days = 30` means one calendar month from billing_start (same as client `computeBillingCycleEnd`).
...
begin
  ...
  if coalesce(cycle_days, 0) = 30 then
    cycle_end := billing_start + interval '1 month';
  else
    cycle_end := billing_start + (cycle_days || ' days')::interval;
  end if;
