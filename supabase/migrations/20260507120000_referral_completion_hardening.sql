-- Harden referral completion: trim + case-insensitive email and referral_code matching,
-- and UUID-based reward tokens (no pgcrypto gen_random_bytes dependency).
-- Prevents silent failures from the exception handler in mark_referral_invite_completed.
...
begin
  ...
  begin
    ...
    where status = 'pending'
      ...
      and (
        ref_code is null
        or lower(trim(referral_code)) = ref_code
      );
  exception
    when others then
...
begin
  ...
  where ri.status = 'pending'
    ...
    and (
      nullif(lower(trim(coalesce(au.raw_user_meta_data ->> 'referral_code', ''))), '') is null
      or lower(trim(ri.referral_code)) = nullif(lower(trim(coalesce(au.raw_user_meta_data ->> 'referral_code', ''))), '')
    );
