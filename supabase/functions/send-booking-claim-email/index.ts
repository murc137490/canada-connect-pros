...
const SUPPORT_EMAIL = "premiereservicescontact@gmail.com";
...
async function sendEmailViaSmtp(
  ...
  try {
    ...
    // denomailer send accepts either single or list for `to` depending on runtime;
    // we send one-by-one to keep it safe.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!claimType || !["refund", "redo", "partial"].includes(claimType)) {
    ...
    const subject = `Premiere Services – Booking claim (${claimType}) #${bookingId}`;
    const html = `
      <h2 style="margin:0 0 12px 0;">Booking claim submitted</h2>
      ...
      <p style="color:#666; font-size:12px;">Submitted from the Premiere Services booking help flow.</p>
    `;
